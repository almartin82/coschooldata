...
# Check if we can actually fetch enrollment data
# The CDE data server (www.cde.state.co.us) is separate from the info pages
# and may be down even when the main site is up
can_fetch <- tryCatch({
  # Try to fetch actual data for the most recent year
  test_data <- coschooldata::fetch_enr(2024, tidy = FALSE)
  !is.null(test_data) && nrow(test_data$school) > 0
