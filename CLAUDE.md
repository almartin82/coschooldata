...
`total_enrollment`, `male`, `female`, `native_american`, `asian`,
`black`, `hispanic`, `white`, `pacific_islander`, `multiracial`

**NOT in tidy enrollment:** No special population subgroups
(econ_disadv, lep, special_ed) are available in the CDE enrollment data.
Gender (male/female) is computed by summing race-gender
cross-tabulations.
...
**Common trap:** Raw CDE data uses codes like `"ALL GRADE LEVELS"`,
`"004"` (PK), `"007"` (K), `"010"` (1st), etc. The
[`process_enr()`](https://almartin82.github.io/coschooldata/reference/process_enr.md)
function normalizes these to `PK`, `K`, `01`, etc. Always filter on the
normalized values.

### entity flags

No `id_enr_aggs()` function. The `aggregation_flag` column is added by
...
`"district"` – has district_id only (campus_id == “0000”) - `"state"` –
fallback

Note: No boolean `is_state`, `is_district`, `is_campus`, or `is_charter`
flags. Use `aggregation_flag` or the `type` column (`"District"` or
`"Campus"`) instead.

------------------------------------------------------------------------

This package provides access to Colorado school enrollment and
assessment data from the Colorado Department of Education (CDE).
...
- **Package cannot download enrollment or assessment data** - all Excel
  files are on the DOWN server
- **[`fetch_enr()`](https://almartin82.github.io/coschooldata/reference/fetch_enr.md)
  and
  [`fetch_assessment()`](https://almartin82.github.io/coschooldata/reference/fetch_assessment.md)
...
CMAS (Colorado Measures of Academic Success) is Colorado’s state
assessment system.
...
Example URLs: - ELA 2024:
`https://www.cde.state.co.us/assessment/2024_cmas_ela_disaggregatedachievementresults` -
Math 2024:
`https://www.cde.state.co.us/assessment/2024_cmas_math_disaggregatedachievementresults` -
Science 2024:
`https://www.cde.state.co.us/assessment/2024_cmas_science_disaggregatedachievementresults`
